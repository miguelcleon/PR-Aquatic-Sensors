...
##need to preserve all datetime records and sue that to merge
...
qsf <- merge(qsq,no,by = "DATETIME", all.x = TRUE)
...
qspulse2 <- subset(qspulse, select = c("DATETIME","discharge_cms","Stage_m",
                                   "SpecCond_final","TurbNTU","FDOM_corrected_QSU",
                                   "NO3","TempC","DOConc_Adj","CondmS","DOPer_Sat","Light_Lux"))

write_csv(qspulse2, path="PR_QS_2019-06-25_XX.csv")
